<script lang="ts">
	...
	let editHealth = $state<BenefitConfig>({
		enabled: false,
		employeeDeduction: 0,
		employerContribution: 0,
		isTaxable: false
	});
	let editDental = $state<BenefitConfig>({
		enabled: false,
		employeeDeduction: 0,
		employerContribution: 0,
		isTaxable: false
	});
	let editVision = $state<BenefitConfig>({
		enabled: false,
		employeeDeduction: 0,
		employerContribution: 0,
		isTaxable: false
	});
	let editLifeInsurance = $state<LifeInsuranceConfig>({
		enabled: false,
		employeeDeduction: 0,
		employerContribution: 0,
		isTaxable: false,
	...
	let editDisability = $state<BenefitConfig>({
		enabled: false,
		employeeDeduction: 0,
		employerContribution: 0,
		isTaxable: false
	...
	const totalEmployeeDeduction = $derived(() => {
		...
		if (payGroup.groupBenefits.health.enabled)
			total += payGroup.groupBenefits.health.employeeDeduction;
		if (payGroup.groupBenefits.dental.enabled)
			total += payGroup.groupBenefits.dental.employeeDeduction;
		if (payGroup.groupBenefits.vision.enabled)
			total += payGroup.groupBenefits.vision.employeeDeduction;
		if (payGroup.groupBenefits.lifeInsurance.enabled)
			total += payGroup.groupBenefits.lifeInsurance.employeeDeduction;
		if (payGroup.groupBenefits.disability.enabled)
			total += payGroup.groupBenefits.disability.employeeDeduction;
		return total;
	});
	...
	const totalEmployerContribution = $derived(() => {
		let total = 0;
		if (payGroup.groupBenefits.health.enabled)
			total += payGroup.groupBenefits.health.employerContribution;
		if (payGroup.groupBenefits.dental.enabled)
			total += payGroup.groupBenefits.dental.employerContribution;
		if (payGroup.groupBenefits.vision.enabled)
			total += payGroup.groupBenefits.vision.employerContribution;
		if (payGroup.groupBenefits.lifeInsurance.enabled)
			total += payGroup.groupBenefits.lifeInsurance.employerContribution;
		if (payGroup.groupBenefits.disability.enabled)
			total += payGroup.groupBenefits.disability.employerContribution;
		return total;
	});
	...
	const benefitTypes = [
		{
			...
			label: 'Health',
			icon: 'fa-heart',
			description: 'Medical, prescription drugs'
		},
		...
		{ key: 'vision', label: 'Vision', icon: 'fa-eye', description: 'Eye exams, glasses, contacts' },
		{
			...
			icon: 'fa-shield-alt',
			description: 'Death benefit coverage'
		},
		{
			key: 'disability',
			label: 'Disability',
...
<section class="bg-white rounded-xl shadow-md3-1 overflow-hidden">
	...
	<div class="p-5" ondblclick={handleDoubleClick}>
		...
		{#if isEditing}
			...
			<div class="flex flex-col gap-4">
				...
				{#if editEnabled}
					<div class="flex flex-col gap-3">
						<!-- Health -->
						<div
							class="p-4 rounded-lg border transition-[150ms] {editHealth.enabled
								? 'bg-white border-surface-200'
								: 'bg-surface-50 border-surface-200'}"
						>
							<div class="mb-3">
								<label
									class="flex items-center gap-2 cursor-pointer text-body-content font-medium text-surface-800"
								>
									<input
										type="checkbox"
										class="w-[18px] h-[18px] accent-primary-500"
										bind:checked={editHealth.enabled}
									/>
							...
							{#if editHealth.enabled}
								<div class="flex flex-col gap-3 pl-6 border-l-2 border-primary-100">
									<div class="grid grid-cols-2 gap-3 max-md:grid-cols-1">
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												<span
													class="flex items-center px-2 bg-surface-100 border border-surface-200 border-r-0 rounded-l-md text-body-content text-surface-500"
													>$</span
												>
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editHealth.employeeDeduction}
													min="0"
													step="0.01"
												/>
											</div>
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editHealth.employerContribution}
													min="0"
													step="0.01"
												/>
											</div>
									...
									<label
										class="flex items-center gap-2 text-auxiliary-text text-surface-600 cursor-pointer"
									>
										<input
											type="checkbox"
											class="w-4 h-4 accent-warning-500"
											bind:checked={editHealth.isTaxable}
										/>
						...
						<!-- Dental -->
						<div
							class="p-4 rounded-lg border transition-[150ms] {editDental.enabled
								? 'bg-white border-surface-200'
								: 'bg-surface-50 border-surface-200'}"
						>
							<div class="mb-3">
								...
								>
									<input
										type="checkbox"
										class="w-[18px] h-[18px] accent-primary-500"
										bind:checked={editDental.enabled}
									/>
								...
								<div class="flex flex-col gap-3 pl-6 border-l-2 border-primary-100">
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editDental.employeeDeduction}
													min="0"
													step="0.01"
												/>
											</div>
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editDental.employerContribution}
													min="0"
													step="0.01"
												/>
											</div>
									...
									>
										<input
											type="checkbox"
											class="w-4 h-4 accent-warning-500"
											bind:checked={editDental.isTaxable}
										/>
										<span>Employer portion is taxable benefit</span>
									</label>
								</div>
						...
						<!-- Vision -->
						<div
							class="p-4 rounded-lg border transition-[150ms] {editVision.enabled
								? 'bg-white border-surface-200'
								: 'bg-surface-50 border-surface-200'}"
						>
							<div class="mb-3">
								...
								>
									<input
										type="checkbox"
										class="w-[18px] h-[18px] accent-primary-500"
										bind:checked={editVision.enabled}
									/>
								...
								<div class="flex flex-col gap-3 pl-6 border-l-2 border-primary-100">
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editVision.employeeDeduction}
													min="0"
													step="0.01"
												/>
											</div>
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editVision.employerContribution}
													min="0"
													step="0.01"
												/>
											</div>
									...
									>
										<input
											type="checkbox"
											class="w-4 h-4 accent-warning-500"
											bind:checked={editVision.isTaxable}
										/>
										<span>Employer portion is taxable benefit</span>
									</label>
								</div>
						...
						<!-- Life Insurance -->
						<div
							class="p-4 rounded-lg border transition-[150ms] {editLifeInsurance.enabled
								? 'bg-white border-surface-200'
								: 'bg-surface-50 border-surface-200'}"
						>
							<div class="mb-3">
								...
								>
									<input
										type="checkbox"
										class="w-[18px] h-[18px] accent-primary-500"
										bind:checked={editLifeInsurance.enabled}
									/>
								...
								<div class="flex flex-col gap-3 pl-6 border-l-2 border-primary-100">
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editLifeInsurance.employeeDeduction}
													min="0"
													step="0.01"
												/>
											</div>
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editLifeInsurance.employerContribution}
													min="0"
													step="0.01"
												/>
											</div>
									...
									<div class="grid grid-cols-2 gap-3 max-md:grid-cols-1">
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editLifeInsurance.coverageAmount}
													min="0"
													step="1000"
												/>
											</div>
										...
										<div class="flex flex-col gap-1">
											...
											<select
												class="py-2 px-3 border border-surface-200 rounded-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
												bind:value={editLifeInsurance.coverageMultiplier}
											>
									...
									>
										<input
											type="checkbox"
											class="w-4 h-4 accent-warning-500"
											bind:checked={editLifeInsurance.isTaxable}
										/>
										<span>Employer portion is taxable benefit</span>
									</label>
								</div>
						...
						<!-- Disability -->
						<div
							class="p-4 rounded-lg border transition-[150ms] {editDisability.enabled
								? 'bg-white border-surface-200'
								: 'bg-surface-50 border-surface-200'}"
						>
							<div class="mb-3">
								...
								>
									<input
										type="checkbox"
										class="w-[18px] h-[18px] accent-primary-500"
										bind:checked={editDisability.enabled}
									/>
								...
								<div class="flex flex-col gap-3 pl-6 border-l-2 border-primary-100">
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editDisability.employeeDeduction}
													min="0"
													step="0.01"
												/>
											</div>
										...
										<div class="flex flex-col gap-1">
											...
											<div class="flex items-stretch">
												...
												<input
													type="number"
													class="flex-1 min-w-0 py-2 px-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
													bind:value={editDisability.employerContribution}
													min="0"
													step="0.01"
												/>
											</div>
									...
									>
										<input
											type="checkbox"
											class="w-4 h-4 accent-warning-500"
											bind:checked={editDisability.isTaxable}
										/>
										<span>Employer portion is taxable benefit</span>
									</label>
								</div>
			...
			<div
				class="flex items-start gap-2 mt-4 py-3 px-4 bg-info-50 border border-info-200 rounded-md text-auxiliary-text text-info-800"
			>
				<i class="fas fa-calculator text-info-500 mt-0.5"></i>
				<span>
					<strong>Taxable Benefits:</strong> Employer-paid portions of certain benefits (e.g., group life
					insurance over $10,000) are taxable benefits that must be reported on T4s.
				</span>
			</div>
		{:else}
			<!-- View Mode -->
			<div class="cursor-pointer" title="Double-click to edit">
				<div
					class="flex items-center gap-4 p-4 rounded-lg border {payGroup.groupBenefits.enabled
						? 'bg-success-50 border-success-100'
						: 'bg-surface-50 border-surface-100'}"
				>
					<div
						class="w-12 h-12 rounded-full flex items-center justify-center {payGroup.groupBenefits
							.enabled
							? 'bg-success-100'
							: 'bg-surface-200'}"
					>
				...
				{#if payGroup.groupBenefits.enabled}
					<div class="grid grid-cols-2 gap-3 mt-4 max-md:grid-cols-1">
						{#each benefitTypes as benefit (benefit.key)}
							{@const config =
								benefit.key === 'lifeInsurance'
									? payGroup.groupBenefits.lifeInsurance
									: payGroup.groupBenefits[benefit.key]}
							<div
								class="p-3 rounded-md border {config.enabled
									? 'bg-white border-success-200'
									: 'bg-surface-50 border-surface-100'}"
							>
								<div class="flex items-center gap-2 mb-2">
									<i
										class="fas {benefit.icon} {config.enabled
											? 'text-success-500'
											: 'text-surface-400'}"
									></i>
									<span class="flex-1 text-body-content font-medium text-surface-800"
										>{benefit.label}</span
									>
									<span
										class="py-1 px-2 rounded-full text-caption-text uppercase font-semibold {config.enabled
								...
								{#if config.enabled}
									<div class="flex gap-4 mb-2">
										<div class="flex flex-col gap-0.5">
											<span class="text-caption-text uppercase text-surface-400">Employee</span>
											<span class="text-body-content font-medium text-surface-700"
												>{formatCurrency(config.employeeDeduction)}/pay</span
											>
										</div>
										<div class="flex flex-col gap-0.5">
											<span class="text-caption-text uppercase text-surface-400">Employer</span>
											<span class="text-body-content font-medium text-surface-700"
												>{formatCurrency(config.employerContribution)}/pay</span
											>
										</div>
									</div>
									{#if config.isTaxable}
										<span
											class="inline-block py-0.5 px-2 bg-warning-100 text-warning-700 rounded-sm text-caption-text font-medium"
											>Taxable Benefit</span
										>
									{/if}
								{/if}
							</div>
					...
					<div
						class="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-surface-200 max-md:grid-cols-1"
					>
						<div class="flex flex-col gap-1 p-3 bg-primary-50 rounded-md">
							...
							<span class="text-title-medium font-bold text-primary-700"
								>{formatCurrency(totalEmployeeDeduction())}/pay period</span
							>
						</div>
						<div class="flex flex-col gap-1 p-3 bg-primary-50 rounded-md">
							<span class="text-auxiliary-text text-surface-500">Total Employer Contribution</span>
							<span class="text-title-medium font-bold text-primary-700"
								>{formatCurrency(totalEmployerContribution())}/pay period</span
							>
						</div>
					</div>
				{:else}
