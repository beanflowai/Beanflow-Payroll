<script lang="ts">
	...
	const isValid = $derived(
		!editEnabled || (editIndustryClassCode.length > 0 && editAssessmentRate > 0)
	);
</script>

<section class="bg-white rounded-xl shadow-md3-1 overflow-hidden">
	<div
		class="flex justify-between items-center p-4 px-5 bg-surface-50 border-b border-surface-100 max-md:flex-col max-md:gap-3 max-md:items-start"
	>
	...
	<div class="p-5" ondblclick={handleDoubleClick}>
		...
		{#if isEditing}
					...
					<label class="flex items-start gap-3 cursor-pointer">
						...
						<span class="flex flex-col gap-1">
							<span class="text-body-content font-medium text-surface-800"
								>Enable WCB for this Pay Group</span
							>
					...
					<div class="flex flex-col gap-4 pl-4 border-l-[3px] border-warning-200">
						<div class="grid grid-cols-2 gap-4 max-md:grid-cols-1">
							<div class="flex flex-col gap-1">
								<label
									for="industryClassCode"
									class="text-auxiliary-text font-medium text-surface-600"
									>Industry Class Code *</label
								>
								<input
									type="text"
									...
									class="p-3 border border-surface-200 rounded-md text-body-content text-surface-800 bg-white transition-[150ms] focus:outline-none focus:border-primary-400 focus:ring-[3px] focus:ring-primary-100 {editIndustryClassCode.length ===
									0
										? 'border-error-400'
										: ''}"
								/>
							...
							<div class="flex flex-col gap-1">
								<label for="industryName" class="text-auxiliary-text font-medium text-surface-600"
									>Industry Name</label
								>
								<input
									type="text"
								...
								/>
								<p class="text-auxiliary-text text-surface-500 mt-1">
									Description of your industry classification
								</p>
							</div>
						</div>
						...
						<div class="grid grid-cols-2 gap-4 max-md:grid-cols-1">
							<div class="flex flex-col gap-1">
								<label for="assessmentRate" class="text-auxiliary-text font-medium text-surface-600"
									>Assessment Rate (per $100) *</label
								>
								<div class="flex items-stretch">
									...
									<input
										...
										step="0.01"
										placeholder="0.00"
										class="flex-1 p-3 border border-surface-200 rounded-r-md text-body-content text-surface-800 bg-white transition-[150ms] focus:outline-none focus:border-primary-400 focus:ring-[3px] focus:ring-primary-100 {editAssessmentRate <=
										0
											? 'border-error-400'
											: ''}"
									/>
							...
							<div class="flex flex-col gap-1">
								<label for="maxEarnings" class="text-auxiliary-text font-medium text-surface-600"
									>Maximum Assessable Earnings</label
								>
								<div class="flex items-stretch">
									<span
										class="flex items-center px-3 bg-surface-100 border border-surface-200 border-r-0 rounded-l-md text-body-content text-surface-500"
			...
			>
				...
				<span>
					<strong>Provincial Variations:</strong> WCB is administered provincially. In Ontario it's called
					WSIB, in BC it's WorkSafeBC, etc. Rates and maximum assessable earnings vary by province.
				</span>
			</div>
		{:else}
			<!-- View Mode -->
			<div class="cursor-pointer" title="Double-click to edit">
				<div
					class="flex items-center gap-4 p-4 rounded-lg border {payGroup.wcbConfig.enabled
						? 'bg-warning-50 border-warning-100'
						: 'bg-surface-50 border-surface-100'}"
				>
					<div
						class="w-12 h-12 rounded-full flex items-center justify-center {payGroup.wcbConfig
							.enabled
							? 'bg-warning-100'
							: 'bg-surface-200'}"
					>
				...
				{#if payGroup.wcbConfig.enabled}
					<div class="mt-4 flex flex-col gap-4">
						<div class="py-3 px-4 bg-primary-50 rounded-md flex flex-col gap-1">
							...
							<div class="flex flex-col gap-1">
								<span class="text-title-medium font-bold text-primary-700 font-mono"
									>{payGroup.wcbConfig.industryClassCode ?? '—'}</span
								>
								{#if payGroup.wcbConfig.industryName}
									<span class="text-body-content text-surface-700"
										>{payGroup.wcbConfig.industryName}</span
									>
								{/if}
							</div>
						</div>
						...
						<div
							class="flex items-center gap-2 py-3 px-4 bg-surface-100 rounded-md text-auxiliary-text text-surface-600"
						>
							<i class="fas fa-calculator text-primary-500"></i>
							<span>
								Estimated annual premium for an employee earning $50,000:
								<strong>
									{formatCurrencyNoDecimals((50000 / 100) * payGroup.wcbConfig.assessmentRate)}
								</strong>
							</span>
						</div>
					</div>
				{:else}
					<div
						class="flex items-start gap-3 mt-4 p-4 bg-surface-50 rounded-md border border-dashed border-surface-200"
					>
						<i class="fas fa-exclamation-triangle text-warning-500 text-xl"></i>
						<div>
							<p class="m-0 text-body-content text-surface-600">
								WCB coverage is not enabled for this pay group. Employees will not have WCB premiums
								calculated.
							</p>
			...
			<p
				class="flex items-center gap-2 mt-4 pt-4 border-t border-dashed border-surface-200 text-auxiliary-text text-surface-400"
			>
