...
<div class="flex flex-col gap-8">
	...
	{#if isLoading}
		<div class="flex flex-col items-center justify-center py-12 px-6 gap-4">
			<div
				class="w-10 h-10 border-[3px] border-surface-200 border-t-primary-500 rounded-full animate-spin"
			></div>
	...
	{:else}
		...
		{#if isNewCompany}
			<div
				class="flex items-center gap-3 p-4 bg-primary-50 border border-primary-200 rounded-lg text-primary-700"
			>
		...
		<section class="flex flex-col gap-4">
			<div class="flex items-start gap-4">
				<div
					class="w-10 h-10 rounded-lg bg-primary-100 text-primary-600 flex items-center justify-center text-lg shrink-0"
				>
			...
			<div class="bg-white rounded-xl shadow-md3-1 p-6">
				...
				<div class="flex flex-col gap-3 mb-6 pb-6 border-b border-surface-100">
					...
					<div class="flex items-start gap-4">
						<!-- Logo Preview -->
						<div
							class="w-24 h-24 rounded-lg border-2 border-dashed border-surface-200 bg-surface-50 flex items-center justify-center overflow-hidden shrink-0"
						>
							{#if logoUrl}
				...
				<div class="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-5">
					<div class="flex flex-col gap-2">
						<label class="text-body-content font-medium text-surface-700" for="company-name"
							>Company Name *</label
						>
						<input
							type="text"
					...
					<div class="flex flex-col gap-2">
						<label class="text-body-content font-medium text-surface-700" for="business-number"
							>Business Number (BN) *</label
						>
						<input
							type="text"
							id="business-number"
						...
						<span class="text-auxiliary-text text-surface-500"
							>Your 9-digit CRA business number</span
						>
					</div>

					<div class="flex flex-col gap-2">
						<label class="text-body-content font-medium text-surface-700" for="payroll-account"
							>Payroll Account Number *</label
						>
						<input
							type="text"
							id="payroll-account"
						...
						/>
						<span class="text-auxiliary-text text-surface-500"
							>15-character CRA payroll account (e.g., 123456789RP0001)</span
						>
					</div>

					<div class="flex flex-col gap-2">
						<label class="text-body-content font-medium text-surface-700" for="province"
		...
		<section class="flex flex-col gap-4">
			<div class="flex items-start gap-4">
				<div
					class="w-10 h-10 rounded-lg bg-secondary-100 text-secondary-600 flex items-center justify-center text-lg shrink-0"
				>
			...
			<div class="bg-white rounded-xl shadow-md3-1 p-6">
				<div class="flex flex-col gap-2 mb-4">
					<label class="text-body-content font-medium text-surface-700" for="remitter-type"
						>Remitter Type *</label
					>
					<select
						id="remitter-type"
						class="py-3 px-4 bg-white border border-surface-200 rounded-lg text-body-content text-surface-800 transition-[150ms] focus:outline-none focus:border-primary-400 focus:ring-[3px] focus:ring-primary-100 max-w-[400px]"
						bind:value={remitterType}
					>
					...
					<span class="text-auxiliary-text text-surface-500"
						>Based on your Average Monthly Withholding Amount (AMWA)</span
					>
				</div>
				...
				{#if showRemitterInfo}
					<div class="bg-surface-50 rounded-lg p-4 mb-4">
						...
						<div class="flex flex-col gap-2 mb-4">
							{#each Object.entries(REMITTER_TYPE_INFO) as [value, info] (value)}
								<div
									class="bg-white border rounded-md p-3 transition-[150ms] {remitterType === value
						...
						<p class="flex items-start gap-2 text-auxiliary-text text-surface-600 m-0">
							...
							Check your CRA My Business Account or the letter CRA sent you to confirm your remitter type.
						</p>
					</div>
				{/if}
				...
				<div class="border-t border-surface-100 pt-4 mt-4">
					<h3
						class="text-auxiliary-text font-medium text-surface-500 uppercase tracking-wider m-0 mb-3"
					>
						Current Status
					</h3>
					<div class="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
						<div class="bg-surface-50 rounded-lg p-4 flex flex-col gap-1">
							...
							<span class="text-title-medium font-semibold text-surface-800"
								>{REMITTER_TYPE_INFO[remitterType].frequency}</span
							>
						...
						<div class="bg-surface-50 rounded-lg p-4 flex flex-col gap-1">
							...
							<span class="text-title-medium font-semibold text-surface-800"
								>{dueDateInfo().formatted}</span
							>
							<span
								class="text-auxiliary-text flex items-center gap-1 {dueDateInfo().daysRemaining <= 7
									? 'text-warning-600'
									: 'text-surface-600'}"
							>
		...
		<section class="flex flex-col gap-4">
			<div class="flex items-start gap-4">
				<div
					class="w-10 h-10 rounded-lg bg-tertiary-100 text-tertiary-600 flex items-center justify-center text-lg shrink-0"
				>
			...
			<div class="bg-white rounded-xl shadow-md3-1 p-6">
				<div class="flex items-center justify-between py-4 border-b border-surface-100">
					<div class="flex-1">
						<span
							id="auto-calc-label"
							class="block text-body-content font-medium text-surface-800 mb-1"
							>Auto-calculate deductions</span
						>
						<span class="text-auxiliary-text text-surface-600">
					...
					<span class="relative inline-block w-12 h-7 shrink-0">
						<input
							...
							aria-labelledby="auto-calc-label"
						/>
						<span
							class="absolute cursor-pointer inset-0 bg-surface-300 transition-[150ms] rounded-full before:absolute before:content-[''] before:h-5 before:w-5 before:left-1 before:bottom-1 before:bg-white before:transition-[150ms] before:rounded-full {autoCalculate
								? 'bg-primary-500 before:translate-x-5'
								: ''}"
						></span>
					</span>
				</div>
				...
				<div class="flex items-center justify-between py-4">
					...
					<span class="relative inline-block w-12 h-7 shrink-0">
						<input
							type="checkbox"
							class="opacity-0 w-0 h-0"
							bind:checked={sendPaystubs}
							aria-labelledby="paystub-label"
						/>
						<span
							class="absolute cursor-pointer inset-0 bg-surface-300 transition-[150ms] rounded-full before:absolute before:content-[''] before:h-5 before:w-5 before:left-1 before:bottom-1 before:bg-white before:transition-[150ms] before:rounded-full {sendPaystubs
								? 'bg-primary-500 before:translate-x-5'
								: ''}"
						></span>
					</span>
		...
		<div class="flex justify-end gap-3 pt-6 border-t border-surface-200 max-sm:flex-col">
			<button
				class="inline-flex items-center justify-center gap-2 py-3 px-5 border border-surface-200 rounded-lg text-body-content font-medium cursor-pointer transition-[150ms] bg-white text-surface-700 hover:bg-surface-50 hover:border-surface-300 disabled:opacity-60 disabled:cursor-not-allowed max-sm:w-full"
				onclick={onCancel}
				disabled={isSaving}>Cancel</button
			>
			<button
				class="inline-flex items-center justify-center gap-2 py-3 px-5 border-none rounded-lg text-body-content font-medium cursor-pointer transition-[150ms] bg-gradient-to-br from-primary-600 to-secondary-600 text-white shadow-md3-1 hover:opacity-90 hover:-translate-y-px disabled:opacity-60 disabled:cursor-not-allowed max-sm:w-full"
				onclick={handleSave}
				disabled={isSaving}
			>
				{#if isSaving}
