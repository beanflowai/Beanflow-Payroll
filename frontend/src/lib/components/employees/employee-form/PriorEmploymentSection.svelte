<script lang="ts">
	interface HireDateInfo {
		showSection: boolean;
		defaultPriorEmployment: boolean;
		message: string;
	}
		...
		<div class="flex flex-col gap-4">
			...
			{#if hasPriorEmployment}
				...
				<div class="flex flex-col gap-2">
					...
					{:else}
						<div class="p-3 bg-surface-100 rounded-md text-body-content text-surface-600">
							{incomeLevel === 'high' ? `$${highIncomeThreshold.toLocaleString()}/year or above` : `Below $${highIncomeThreshold.toLocaleString()}/year`}
						</div>
					{/if}
				</div>
				...
				{#if incomeLevel === 'low'}
					...
					<div class="flex items-center gap-3 p-3 bg-success-50 border border-success-200 rounded-lg text-success-700 text-body-small">
						...
						<span>No prior CPP/EI input needed - employees below ${highIncomeThreshold.toLocaleString()}/year rarely hit annual maximums.</span>
					</div>
				{:else}
