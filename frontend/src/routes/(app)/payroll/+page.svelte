<script lang="ts">
	...
	// Shared components can be added back when needed
	import {
...
{#if isLoading}
	...
	<div class="flex flex-col items-center justify-center min-h-[400px] text-center">
		<div
			class="w-12 h-12 border-4 border-surface-200 border-t-primary-500 rounded-full animate-spin mb-4"
		></div>
...
{:else if !companyState.currentCompany}
	...
	<div class="flex flex-col items-center justify-center min-h-[400px] text-center">
		<div
			class="w-20 h-20 rounded-full bg-surface-100 text-surface-400 flex items-center justify-center text-headline-medium mb-4"
		>
			<i class="fas fa-building"></i>
		</div>
		<h3 class="text-title-small font-semibold text-surface-800 m-0 mb-2">No Company Selected</h3>
		<p class="text-body-content text-surface-600 m-0 mb-6">
			Please select or create a company to run payroll.
		</p>
	</div>
{:else if error}
	<!-- Error State -->
	<div class="flex flex-col items-center justify-center min-h-[400px] text-center">
		<div
			class="w-20 h-20 rounded-full bg-error-100 text-error-600 flex items-center justify-center text-headline-medium mb-4"
		>
		...
		<button
			class="inline-flex items-center gap-2 py-3 px-5 bg-gradient-to-br from-primary-600 to-secondary-600 text-white border-none rounded-lg text-body-content font-medium cursor-pointer transition-fast hover:opacity-90 hover:-translate-y-px"
			onclick={() => loadData()}
		>
		...
		<header class="mb-6">
			<div>
				...
				<p class="text-body-content text-surface-600 m-0">
					Get started by creating your first pay group
				</p>
			</div>
		</header>

		<div
			class="flex flex-col items-center py-16 px-6 bg-white rounded-xl shadow-md3-1 text-center max-w-[500px] mx-auto"
		>
			<div
				class="w-[100px] h-[100px] rounded-full bg-primary-100 text-primary-600 flex items-center justify-center text-headline-large mb-6"
			>
				<i class="fas fa-layer-group"></i>
			</div>
			<h3 class="text-title-medium font-semibold text-surface-800 m-0 mb-3">No Pay Groups Yet</h3>
			<p class="text-body-content text-surface-600 m-0 mb-8 leading-relaxed">
				Create a pay group to organize employees by pay frequency and configure payroll settings.
			</p>
			<a
				href="/company?tab=pay-groups"
				class="inline-flex items-center gap-2 py-3 px-5 bg-gradient-to-br from-primary-600 to-secondary-600 text-white border-none rounded-lg text-body-content font-medium no-underline cursor-pointer transition-fast hover:opacity-90 hover:-translate-y-px"
			>
	...
	<div class="max-w-[1200px] mx-auto">
		...
		<header class="mb-6">
			<div>
				<h1 class="text-headline-minimum font-semibold text-surface-800 m-0 mb-1">Run Payroll</h1>
				<p class="text-body-content text-surface-600 m-0">
					Manage upcoming payroll runs for all pay groups
				</p>
			</div>
		</header>
		...
		<!-- Quick Stats -->
		<div
			class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-4 mb-8"
		>
			<div class="flex items-center gap-4 p-5 bg-white rounded-xl shadow-md3-1">
				<div
					class="w-12 h-12 rounded-lg flex items-center justify-center text-title-large bg-primary-100 text-primary-600"
				>
			...
			{#if nextPeriod}
				<div class="flex items-center gap-4 p-5 bg-white rounded-xl shadow-md3-1">
					<div
						class="w-12 h-12 rounded-lg flex items-center justify-center text-title-large bg-warning-100 text-warning-600"
					>
						<i class="fas fa-clock"></i>
					</div>
					<div class="flex flex-col">
						<span class="text-title-large font-semibold text-surface-800"
							>{formatShortDate(nextPeriod.periodEnd)}</span
						>
				...
				<div class="flex items-center gap-4 p-5 bg-white rounded-xl shadow-md3-1">
					<div
						class="w-12 h-12 rounded-lg flex items-center justify-center text-title-large bg-tertiary-100 text-tertiary-600"
					>
						<i class="fas fa-users"></i>
					</div>
					<div class="flex flex-col">
						<span class="text-title-large font-semibold text-surface-800"
							>{nextPeriod.totalEmployees}</span
						>
				...
				<div class="flex items-center gap-4 p-5 bg-white rounded-xl shadow-md3-1">
					<div
						class="w-12 h-12 rounded-lg flex items-center justify-center text-title-large bg-success-100 text-success-600"
					>
						<i class="fas fa-dollar-sign"></i>
					</div>
					<div class="flex flex-col">
						<span class="text-title-large font-semibold text-surface-800"
							>{formatCurrency(nextPeriod.totalEstimatedGross, { maximumFractionDigits: 0 })}</span
						>
		...
		<section class="mb-8">
			<div class="flex items-center justify-between mb-4">
				...
				<a
					...
					class="flex items-center gap-2 text-body-content font-medium text-primary-600 no-underline transition-fast hover:text-primary-700"
				>
			...
			{#if upcomingPeriods.length === 0}
				<div
					class="flex flex-col items-center py-12 px-6 bg-white rounded-xl shadow-md3-1 text-center"
				>
					<div
						class="w-20 h-20 rounded-full bg-surface-100 text-surface-400 flex items-center justify-center text-headline-medium mb-4"
					>
					...
					<h3 class="text-title-small font-semibold text-surface-800 m-0 mb-2">
						No Upcoming Pay Periods
					</h3>
					<p class="text-body-content text-surface-600 m-0 mb-6">
						All scheduled payroll runs have been completed.
					</p>
				</div>
			{:else}
							...
							{#each recentRuns as run (run.id)}
								...
								>
									...
									<td
										...
										>{formatCurrency(run.totalNetPay, { maximumFractionDigits: 0 })}</td
									>
									<td
										class="p-4 px-5 text-body-content border-b border-surface-100 text-surface-700 last:border-b-0"
									>
		...
		<section class="mb-8">
			...
			<div class="grid grid-cols-1 md:grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-4">
				<a
					href="/employees"
					class="flex items-center gap-4 p-5 bg-white rounded-xl shadow-md3-1 no-underline transition-fast hover:shadow-md3-2 hover:-translate-y-0.5 group"
				>
					<div
						class="w-11 h-11 rounded-lg bg-surface-100 text-surface-600 flex items-center justify-center text-title-medium shrink-0 group-hover:bg-primary-100 group-hover:text-primary-600"
					>
						<i class="fas fa-user-plus"></i>
					</div>
					<div class="flex-1">
						<h3 class="text-body-content font-semibold text-surface-800 m-0 mb-1">
							Manage Employees
						</h3>
				...
				>
					...
					<div class="flex-1">
						...
						<p class="text-auxiliary-text text-surface-600 m-0">
							Configure pay frequencies and policies
						</p>
					</div>
					<i class="fas fa-chevron-right text-surface-400 text-sm group-hover:text-primary-600"></i>
				</a>
