<script lang="ts">
	...
	import { getMockUpcomingPayDates } from '$lib/mocks/payroll-dashboard';

	// ===========================================
	// State
	// ===========================================
	const upcomingPayDates = $state<UpcomingPayDate[]>(getMockUpcomingPayDates());
	...
	const totalEmployees = $derived(
		upcomingPayDates.reduce((sum, pd) => sum + pd.totalEmployees, 0)
	);
	...
	<section class="pay-dates-section">
		...
		{#if upcomingPayDates.length === 0}
			<div class="empty-state">
				...
				<p>Set up pay groups in Company Settings to start running payroll.</p>
