<script lang="ts">
	...
	import { addMockPayGroup } from '$lib/mocks/pay-groups';
	...
	// Create a new pay group with default values
	const newId = `pg-${Date.now()}`;
	const now = new Date().toISOString();

	let payGroup = $state<PayGroup>({
		...createDefaultPayGroup('company-1'),
		id: newId,
		createdAt: now,
		updatedAt: now
	});
	...
	function handleSave() {
		...
		// Add to mock data (simulate API call)
		setTimeout(() => {
			addMockPayGroup(savedPayGroup);
			isSaving = false;
			// Navigate to the detail page
			goto(`/company/pay-groups/${savedPayGroup.id}`);
		}, 300);
	}
